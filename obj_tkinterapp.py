...
from utils import init_model, process_frame # 导入 utils.py 文件中的 init_model 和 process_frame 函数
...
class Application:
    ...
    def change_model(self):
        ...
        if model_opt1 == "定位":
            ...
            self.model = init_model(model_path)# TODO: 利用utils函数获取模型
            # 用日志记录当前更改的模型名称
            logging.info(f"更改模型为 {model_name+model_opt2}-pose.pt")
        ...
        if self.vid is not None:
            ...
            if ret:
                ...
                if self.detecting:
                    ...
                    # else:
                    #     frame = process_frame(self.model, frame, False, True)
                    frame = process_frame(self.model, frame, True, True)
